...
const AUTH_HEADER = 'Basic YXBwY2xpZW50OmFwcHNlY3JldA==';
...
test('Check after sub create user', async t => {
  ...
  try {
    ...
    t.ok(resp.data.data.patient_id.length > 0);
    const patientId = resp.data.data.patient_id;
    resp = await axios({
      method: 'GET',
      url: `${AIDBOX_URL}/Patient/${patientId}`,
      headers: {
        authorization: AUTH_HEADER,
        accept: 'application/json'
      }
    });
    t.ok(resp.data.id === patientId);
  } catch (err) {
    console.log(err);
