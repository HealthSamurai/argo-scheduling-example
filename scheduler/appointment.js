...
function makeEntry(data, { patient_id, visitType }) {
...
async function findAppointments(ctx, msg) {
  ...
  try {
    ...
    if (!user || !user.data || !user.data.patient_id) {
      return {error: "cannot get current user"};
    }

    const patient_id = user.data.patient_id;
	...
	.map(d => makeEntry(d, { patient_id, visitType }))
    };
  } catch (error) {
